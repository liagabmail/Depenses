    ...
    <div class="modal-body">
      ...
      <div class="depot-formulaire">
        <div class="field"><label for="occ-depot-montant">Montant ($)</label>
          <input type="number" id="occ-depot-montant" data-num="montant" min="0" step="0.01" inputmode="decimal" autocomplete="new-password" readonly data-antifill value="${d.amount}"></div>
      ...
      <div class="depot-formulaire">
        <div class="field"><label for="occ-report-montant">Montant ($)</label>
          <input type="number" id="occ-report-montant" data-num="montant" min="0" step="0.01" inputmode="decimal" autocomplete="new-password" readonly data-antifill value="${d.amount}"></div>
    ...
    <div class="modal-body">
      <div class="modal-grid">
        <div class="field"><label for="dep-mod-montant">Montant ($)</label>
          <input type="number" id="dep-mod-montant" data-num="montant" min="0" step="0.01" inputmode="decimal" autocomplete="new-password" readonly data-antifill value="${d.amount}"></div>
...
/* ===================== ANTI-AUTOFILL (Montant / % / Note) =====================
   Android (clavier + service de saisie automatique du système) propose des suggestions
   d'auto-remplissage sur ces champs même avec autocomplete="off"/"new-password", car ce
   service ignore souvent ces attributs. Ces champs sont donc rendus `readonly` dans le
   HTML (voir attribut data-antifill) : la plupart des moteurs d'auto-remplissage scannent
   la page une seule fois et ignorent alors ces champs. On retire le `readonly` dès que
   l'utilisateur touche le champ (focusin, qui contrairement à `focus` remonte jusqu'ici
   même pour des champs ajoutés dynamiquement), donc la saisie reste normale ensuite. Le
   HTML étant regénéré à chaque ouverture de fenêtre, le `readonly` est remis par défaut
   la fois suivante. Ne touche pas aux <input type="date"> : ceux-ci ont leur propre
   `readonly` permanent pour le sélecteur de date maison (voir plus bas). */
document.addEventListener('focusin', e=>{
  if(e.target.matches && e.target.matches('[data-antifill][readonly]')) e.target.removeAttribute('readonly');
});
